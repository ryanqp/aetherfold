...
func _discover_suites(
	...
	var outcome := _discover_dir(
		"res://tests", suites, errors, service_cb, deadline_ticks_ms, run_state
	)
	if not outcome.is_empty():
		return {"suites": suites, "errors": errors, "outcome": outcome}
...
func _discover_dir(
	...
	if dir == null:
		if dir_path == "res://tests":
			errors.append("DirAccess.open('res://tests') returned null — directory may not exist")
		return ""
	dir.list_dir_begin()
	...
	while not file_name.is_empty():
		...
		if dir.current_is_dir():
			var nested := _discover_dir(
				path, suites, errors, service_cb, deadline_ticks_ms, run_state
			)
			...
			if script == null:
				errors.append("%s (load failed — check for parse errors or duplicate methods)" % path)
			elif script.can_instantiate():
				var instance = script.new()
				if instance is McpTestSuite:
					suites.append(instance)
				else:
					errors.append("%s (not a McpTestSuite subclass)" % path)
			else:
